...
/**
 * Render a user-supplied string as a YAML scalar, quoted only when
 * needed. We hand the value to `yaml`'s own serializer and trim the
 * trailing newline so the result drops cleanly into our template.
 *
 * Without this, values containing `:`, `#`, leading `@`, brackets,
 * quotes, or newlines (literal API keys, paths with spaces, org names
 * with punctuation, …) silently truncate or change type at parse time.
 */
...
export function generateConfig(answers: InitAnswers): string {
  ...
  switch (answers.apiKeySource) {
    case "keychain": {
      ...
      authLine = `    apiKeyKeychain: ${s(svc)}`;
      break;
    }
    case "env": {
      const name = answers.apiKeyEnvName ?? "OBSERVER_API_KEY";
      authLine = `    apiKeyEnv: ${s(name)}`;
      break;
    }
    case "literal":
      authLine = answers.apiKey
        ? `    apiKey: ${s(answers.apiKey)}`
...
${authLine}
    ...
    orgs:
      include:
${answers.includeOrgs.map((o) => `        - ${s(o)}`).join("\n")}` : ""}`
    : "";
...
git:
  enabled: true
  repos: {}
  # Only collect commits authored by ${answers.developer.replace(/\n/g, " ")}. Set to false
