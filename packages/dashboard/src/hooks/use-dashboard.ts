...
export function useValidationLoops(filters: DashboardFilters) {
  const [rows, setRows] = useState<ValidationLoopRow[] | null>(null);
  const { days, project, agent } = filters;

  useEffect(() => {
    let cancelled = false;
    const params = buildParams({ days, project, agent });
    fetchJson<ValidationLoopRow[]>(`/api/validation/loops${params}`)
      .then((d) => { if (!cancelled) setRows(d); })
      .catch(() => { if (!cancelled) setRows([]); });
    return () => { cancelled = true; };
  }, [days, project, agent]);
